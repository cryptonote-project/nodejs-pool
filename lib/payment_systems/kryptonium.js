    ...
    global.support.rpcWallet('getbalance', [], function (body) {
        if (body.result) {
            ...
            if (body.result.unlocked_balance < amountToPay) {
                console.error("Wallet only has " + body.result.unlocked_balance + " unlocked balance, can't pay " + amountToPay + " worth of XSMXSM. Retrying in " + timerRetry + " minutes!");
    ...
    global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
        ...
        if (paymentDetails.hasOwnProperty('payment_id')) {
            ...
            if (transferFunc === 'transfer') {
                console.log("Payment made out to multiple people, total fee: " + global.support.coinToDecimal(body.result.fee) + " XSM");
            }
            let intCount = 0;
            paymentDetails.destinations.forEach(function (details) {
                console.log("Payment made to: " + details.address + " For: " + global.support.coinToDecimal(details.amount) + " XSM");
                intCount += 1;
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                ...
                if (global.config.general.discordPayoutAnnounce === true) {
                    global.support.poolBotMsg("Pool paid out: " + global.support.coinToDecimal(amount)  + " XSM to 1 miner");
                }
            } else {
                    ...
                    while (paymentDestinations.length > 0) {
                        ...
                        paymentQueue.push(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (body.fee && body.fee > 10) {
                                ...
                                if (global.config.general.discordPayoutAnnounce === true) {
                                    global.support.poolBotMsg("Pool paid out: " + global.support.coinToDecimal(totalAmount)  + " XSM to " + paymentDetails.destinations.length + " miners");
                                }
                            } else {
                                console.error("Unknown error from the wallet.");
                            }
